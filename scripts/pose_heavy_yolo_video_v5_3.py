...
# - その bbox を上下左右に SCALE 倍だけ拡大してクロップ
# - そのクロップを MediaPipe PoseLandmarker Heavy (3D) で解析
...
# MediaPipe Image クラス（mp_env / mediapipe==0.10.14 用）
MPImage = mp.Image
...
# ★ YOLO bbox をどれくらい拡大して Heavy に渡すか（中心から SCALE 倍）
CROP_SCALE = 4.0
...
def expand_bbox(
    ...
    """
    YOLO が出した bbox を SCALE 倍に拡大（中心を保ったまま）して返す。
    画像外にはみ出さないようにクリップ。
    """
    cx = (x1 + x2) / 2.0
    cy = (y1 + y2) / 2.0
    ...
    nx1 = max(int(round(cx - w / 2.0)), 0)
    ny1 = max(int(round(cy - h / 2.0)), 0)
    nx2 = min(int(round(cx + w / 2.0)), img_w - 1)
    ny2 = min(int(round(cy + h / 2.0)), img_h - 1)
...
def to_fullframe_landmarks(
    ...
    """
    拡大後クロップの MediaPipe ランドマークを
    フル解像度(例: 3840x2160) の正規化(0-1)へ変換。
    """
    x1, y1, x2, y2 = bbox
    crop_w = max(x2 - x1, 1)
    crop_h = max(y2 - y1, 1)
...
def main():
    parser = argparse.ArgumentParser(
        description="YOLOv8x-pose + MediaPipe Heavy PoseLandmarker 4K 極太骨格 overlay (v5.3, CROP_SCALE=4.0)"
    )
    parser.add_argument(
        "--video",
        required=True,
