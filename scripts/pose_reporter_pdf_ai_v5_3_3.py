...
def load_metrics(csv_path: str, m_per_px: float):
    ...
    # 速度[m/s] 列が無ければ自動生成（dist_px * m_per_px / dt）
...
def extract_pose_frames(video_path: str) -> dict:
    ...
    out_dir = Path("outputs/pose_images") / stem.replace(".", "_")
    ensure_dir(out_dir)
...
def create_metrics_graph(df: pd.DataFrame, m_per_px: float, video_stem: str) -> str:
    ...
    plt.tight_layout()
    out_path = Path("outputs/graphs") / f"{video_stem}_metrics_v5_3_4.png"
...
def create_angles_graph(df_angles: pd.DataFrame, angle_stats: dict, video_stem: str) -> str:
    ...
    if not ordered_cols:
        ...
        ax.axis("off")
        out_path = Path("outputs/graphs") / f"{video_stem}_angles_v5_3_4.png"
        plt.savefig(out_path, dpi=200)
        plt.close(fig)
        return str(out_path)
...
def generate_ai_comments(df_metrics: pd.DataFrame, summary: dict, angle_stats: dict) -> list:
    """
    速度波形＋体幹傾斜＋関節角度＋安定性から、
    コーチ研修レベルの専門的コメントを生成する。
    """
    comments = []
    ...
    comments.append(
        ...
        f"加速局面における踏み出しのエネルギー伝達と接地時間のマネジメントが良好であることを示します。"
    )
...
def build_pdf(csv_path: str, video_path: str, athlete: str):
    ...
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.3.4）", ln=1)
    ...
    pdf.cell(0, 8, "速度・重心・角度の時間変化", ln=1)
    ...
    pdf.multi_cell(
        ...
        "『どの局面で推進効率が高いか／ロスが生じているか』を細かく評価できます。"
    )
    ...
    out_pdf = Path("outputs/pdf") / f"{athlete}_form_report_v5_3_4.pdf"
...
def main():
    parser = argparse.ArgumentParser(description="KJAC テクニカルAI v5.3.4 単走フォームレポート")
