# scripts/patch_fix_src_calibration_interactive.py
# -*- coding: utf-8 -*-
"""
Fix src/calibration.py to match src.cli_one_stage expectations:
- calibrate_interactive(video) -> (mpp, px, rm)
Keeps:
- load_scale(scales_dir, cam_name) -> (mpp, raw)
- save_scale(scales_dir, cam_name, mpp, raw=None) -> Path
...
def main():
    ...
    bak = CALIB.with_name(CALIB.name + f".bak_calibfix_{STAMP}")
