...
CONFIG_PATH = "pose_gui_config.json"

DEFAULT_CONFIG = {
    "video_path": "",
    "output_dir": "outputs",  # 実際は pose_reporter 側が ./outputs 固定
    "overlay": True,
    "pdf": True,
    "ai": True,
}
...
class AnalyzerBackend:
    """
    ・動画(mp4)を1本受け取り
    ・pose_metrics_analyzer_v3_4.py で CSV 自動生成
    ・pose_reporter_pdf_ai_v5_5_3.build_pdf() で PDF 生成
    """

    # あなたの環境に合わせたパス（B と回答済み）
    ...
    def run_analysis(self, video_path, output_dir, options, progress_callback=None):
        import subprocess
        ...
        # athlete 名はとりあえずファイル名（拡張子なし）にする
        ...
        # pose_reporter 側の標準の出力構造を使う (outputs/<athlete>/<video_id>/...)
...
class Worker(QThread):
    ...
    def __init__(self, backend: AnalyzerBackend, video_path, output_dir, options, parent=None):
...
class MainWindow(QMainWindow):
    ...
    def _create_io_group(self) -> QGroupBox:
        ...
        # 出力ルート（参考表示：実際は pose_reporter 側が ./outputs 固定）
        self.le_output = QLineEdit("outputs")
        self.le_output.setReadOnly(True)
        form.addRow("出力ルート（固定）", self.le_output)
    ...
    def _create_option_group(self) -> QGroupBox:
        ...
        self.cb_ai = QCheckBox("AI コメントを生成（pose_reporter v5.5.3 に依存）")
        self.cb_ai.setChecked(True)

        # 現状、これらの ON/OFF は pose_reporter 側に直結していないが、
        # 将来の拡張用に UI と設定値だけ持たせておく
    ...
    def _load_initial_config(self):
        ...
        self.le_output.setText(cfg.get("output_dir", "outputs"))
    ...
    def _on_run_clicked(self):
        ...
        started = self._start_worker(video_path, output_dir, options, show_busy_dialog=True)
        if started:
    ...
    def _start_worker(self, video_path, output_dir, options, show_busy_dialog: bool) -> bool:
        if self.current_worker is not None and self.current_worker.isRunning():
            if show_busy_dialog:
                QMessageBox.information(self, "処理中", "現在解析中です。完了をお待ちください。")
            return False

        self.current_worker = Worker(self.backend, video_path, output_dir, options)
    ...
    def _update_preview(self, auto: bool = False):
        # pose_reporter の標準出力ルート ./outputs をサーチ
