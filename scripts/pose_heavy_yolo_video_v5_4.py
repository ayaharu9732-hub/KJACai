# ============================================================
# pose_heavy_yolo_video_v5_4.py  （mp_env / mediapipe 0.10.14 用）
#
# - YOLOv8x-pose でメイン選手のバウンディングボックスを取得
# - その領域を MediaPipe PoseLandmarker Heavy (3D) で解析
# - Heavy が取れなかったフレームは「フルフレーム Heavy」にフォールバック
# - 4K 向け極太骨格 + 足部ラインをフルフレームに overlay
# - ランドマーク情報を CSV に出力
#
# 使い方:
#   (mp_env) python scripts/pose_heavy_yolo_video_v5_4.py --video "C:\...\sample.mp4"
# ============================================================
...
CROP_SCALE = 4.0       # YOLO bbox 拡大倍率（基本）
...
def expand_bbox(
    ...
    nx2 = min(int(cx + w / 2), full_w - 1)
    ny2 = min(int(cy + h / 2), full_h - 1)
...
def run(video_path: str) -> None:
    ...
    for result in yolo_model.track(
        ...
        # Heavy が 0 件 → フルフレームで再トライ
        if (pose_result is None or
            pose_result.pose_landmarks is None or
    ...
    log(f"=== pose_heavy_yolo_video_v5_4 完了 (処理時間: {elapsed/60:.1f} 分) ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="YOLOv8x-pose + MediaPipe Heavy PoseLandmarker 4K 極太骨格 overlay (v5.4)"
    )
