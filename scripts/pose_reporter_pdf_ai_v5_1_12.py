# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_12.py
# ✅ グラフ文字も黒化 / 全テキスト左揃え / 白背景統一
# ==========================================================
import os
import json
import textwrap
...
def create_graphs(df, athlete):
    ...
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_12.png"
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(page_w, 12, f"{athlete} フォーム分析レポート（v5.1.12）",
    ...
    # 出力
    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_12.pdf")
    pdf.output(out_path)
    print(f"✅ PDF出力完了: {out_path}")
    print("🖤 全文黒文字（本文＋グラフ含む）・左揃え・白背景です。")
