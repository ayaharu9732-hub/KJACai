...
# ============================================
#  AIコメント生成ロジック（ローカルルールベース）
# ============================================


def safe_text_block(pdf: FPDF, text: str, line_height: float = 5.0, max_width: float = None):
    """
    完全安全な日本語テキスト描画。
    FPDF の multi_cell を一切使わず、cell のみで出力する方式。
    """
    if text is None:
        return
    ...
    for raw_line in lines:
        ...
        for ch in line:
            ...
            if pdf.get_string_width(test) <= max_width:
                ...
                pdf.cell(0, line_height, buf, ln=1)
                buf = ch
...
def jp_wrap(text: str, max_chars: int = 35) -> str:
    """
    日本語の長文をFPDFのmulti_cellで安全に表示できるよう、
    ある程度の文字数で自動改行を入れる。
    （※「スペースが無い長い1語」問題を避けるための前処理）
    """
    if not text:
        return ""
...
def safe_multi_cell(pdf: FPDF, text: str, h: float = 5.0) -> None:
    """
    FPDFのmulti_cellで発生する
      "Not enough horizontal space to render a single character"
    を完全に潰すためのラッパー。

    - 行ごとに処理
    - pdf.get_string_width() で、1行の幅がセル幅を超えないように
      文字単位で強制分割する
    """
    if text is None:
        return
    ...
    for raw_line in lines:
        ...
        for ch in line:
            test = buf + ch
            # この文字を追加しても横幅以内ならバッファに追加
            if pdf.get_string_width(test) <= max_width:
                buf = test
            else:
                # もし buf が空なら、この1文字だけでもはみ出すケース
                # → それでも一度出力してしまって、レイアウトは妥協
                if buf == "":
...
for line in pro_comment.split("\n"):
    safe_text_block(pdf, line, line_height=5)
...
def main():
    parser = argparse.ArgumentParser(description=f"KJAC フォーム分析レポート {VERSION_STR}（高機能ラボ版）")
