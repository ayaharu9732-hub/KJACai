...
def to_ascii_folder(name: str) -> str:
    ...
    例）"二村遥香10.5" → "_10_5"
    本当はローマ字にしたいが、ここでは安全性重視で非英数字を全て '_' に。
    """
...
def create_graphs_from_csv(df: pd.DataFrame, video_name_clean: str, ascii_folder: str) -> str:
    ...
    graph_path = os.path.join(graphs_dir, f"{ascii_folder}_graphs_v5_2_3.png")
...
def build_pdf(csv_path: str, video_path: str, athlete: str):
    ...
    # タイトル
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.2.3）", ln=1)
    pdf.set_font("JP", "", 11)
    ...
    if graph_path and os.path.exists(graph_path):
        ...
        pdf.ln(usable_width * 0.75 / 3)  # 適度なスペース
    else:
        pdf.ln(5)
        pdf.set_font("JP", "", 11)
    ...
    ensure_dir(out_dir)
    out_pdf = os.path.join(out_dir, f"{athlete}_form_report_v5_2_3.pdf")
    pdf.output(out_pdf)
