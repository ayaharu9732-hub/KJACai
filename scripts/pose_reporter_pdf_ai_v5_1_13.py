# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_13.py
# ✅ グラフ日本語フォントをIPAexGothicで強制指定
# ==========================================================
import os
import json
import textwrap
...
def create_graphs(df, athlete):
    ...
    for ax, col, title, color in zip(axs, cols, titles, colors):
        ...
        ax.set_title(title, fontsize=11, fontweight="bold", fontproperties=prop, color="black")
    ...
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_13.png"
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(page_w, 12, f"{athlete} フォーム分析レポート（v5.1.13）",
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_13.pdf")
    pdf.output(out_path)
    print(f"✅ PDF出力完了: {out_path}")
    print("🖤 グラフタイトル日本語正常化済み（IPAexGothic適用）")
