# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_10.py
# ✅ 右寄り完全修正（new_x=LMARGIN 指定）＋中央タイトル＋日本語フォント
# ==========================================================
import os
import json
import textwrap
...
def create_graphs(df, athlete):
    ...
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_10.png"
...
class PDF(FPDF):
    def __init__(self):
        ...
        self.set_margins(15, 10, 15)  # 左右15mm, 上10mm
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(page_w, 12, f"{athlete} フォーム分析レポート（v5.1.10）",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(2)

    # --- 基本情報（左寄せ / 必ず LMARGIN に戻す） ---
    pdf.set_font_size(11)
    pdf.cell(page_w, 8, f"動画: {os.path.basename(video_path)}",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.cell(page_w, 8, f"解析日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.cell(page_w, 8, f"校正スケール: {m_per_px:.6f} m/px",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.ln(3)

    # --- メトリクス（左寄せ） ---
    pdf.cell(page_w, 8, f"ピッチ: {summary['pitch']:.2f} 歩/s",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.cell(page_w, 8, f"ストライド: {summary['stride']:.2f} m",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.cell(page_w, 8, f"安定性スコア: {summary['stability']:.2f}/10",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")
    pdf.ln(5)
    ...
    for line in wrap_text(summary_text, 45):
        pdf.cell(page_w, 7, line,
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="L")

    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_10.pdf")
    pdf.output(out_path)
    print(f"✅ PDF出力完了: {out_path}")
    print("🎯 全テキストを new_x=LMARGIN で左寄せ固定しました。")
