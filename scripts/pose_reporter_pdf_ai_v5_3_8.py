...
def _save_frame_unicode_safe(frame: np.ndarray, out_path: str) -> bool:
    """
    Windows + OpenCV で日本語パスの cv2.imwrite が失敗する問題を回避するため、
    imencode → Python のファイルIO で保存する。
    """
...
def extract_pose_images_from_overlay(overlay_path: str, out_dir: str) -> Dict[str, str]:
    ...
    for key, idx in indices.items():
        ...
        ok = _save_frame_unicode_safe(frame, out_path)
        if ok:
            result[key] = out_path
        else:
...
def build_pdf(
    ...
    # 日本語フォント設定（通常＋太字の両方を登録）
    font_path = "c:/Windows/Fonts/meiryo.ttc"
    pdf.add_font("JP", "", font_path, uni=True)
    pdf.add_font("JP", "B", font_path, uni=True)
    pdf.set_font("JP", "", 12)

    # タイトル部
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.3.8）", ln=1)
    pdf.set_font("JP", "", 10)
    ...
    if pose_images:
        ...
        pdf.set_y(y_top + img_w * 0.75 + 5)  # 適当に高さ調整
    else:
    ...
    # 保存
    out_pdf = os.path.join(out_dirs["pdf"], f"{athlete}_form_report_v5_3_8.pdf")
    pdf.output(out_pdf)
...
def main():
    parser = argparse.ArgumentParser(description="KJAC フォーム分析レポート v5.3.8（高機能版）")
