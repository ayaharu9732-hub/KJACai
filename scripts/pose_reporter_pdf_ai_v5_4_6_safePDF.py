...
def safe_multi_cell(pdf: FPDF, text: str, h: float = 5.0) -> None:
    ...
    - 常に左マージンから描画する
    - 1 文字ずつ幅を確認し、はみ出す前に改行
    - それでも1文字が大きすぎる場合は、その1文字だけで強制出力
    """
    if not text:
        return
        ...
        for ch in para:
            ...
            if ch_w > max_w:
                ...
                if buf:
                    ...
                    pdf.multi_cell(max_w, h, buf)
                    buf = ""
                pdf.set_x(pdf.l_margin)
                pdf.multi_cell(max_w, h, ch)
                continue

            new_buf = buf + ch
            w = pdf.get_string_width(new_buf)
            if w <= max_w:
                buf = new_buf
            else:
                # ここで改行してから続き
