# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_5.py
# ✅ multi_cell排除＋日本語安全出力版（完全安定版）
# ==========================================================
import os
import json
import pandas as pd
...
def create_graphs(df, athlete):
    ...
    for ax in axs:
        ax.set_xlabel("時間 [s]")
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_5.png"
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.1.5）", new_y="NEXT")
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_5.pdf")
    pdf.output(out_path)
