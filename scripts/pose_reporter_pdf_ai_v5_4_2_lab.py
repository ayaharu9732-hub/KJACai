...
def init_logger(log_dir: str) -> str:
    ...
    log_path = os.path.join(log_dir, f"v5_4_2_lab_{ts}.log")
    LOG_FILE = log_path
    return log_path
...
def load_metrics_csv(csv_path: str) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    if str(df.iloc[0, 0]) == "_summary_":
        ...
        # ピッチ・ストライド・安定性（summary 行に入っている想定）
...
def ensure_angles_csv(video_path: str, athlete: str, video_id: str, angles_dir: str) -> Optional[str]:
    ...
    - 旧形式1: outputs/angles/{athlete}/{video_id}.csv
    - 旧形式2: outputs/angles_動画名.csv
    """
...
def extract_pose_images_from_overlay(overlay_path: str, out_dir: str) -> Dict[str, str]:
    ...
    Windows + 日本語パスでも確実に保存できるよう、
    cv2.imwrite ではなく「imencode → PythonファイルIO」で書き出す。
    """
    result: Dict[str, str] = {}
    ...
    for key, idx in indices.items():
        ...
        try:
            # OpenCV のパス問題回避: imencode → Python IO
...
def build_pdf(
    ...
    angle_csv = ensure_angles_csv(video_path, athlete, get_video_id(video_path), out_dirs["angles"])
    ...
    metrics_graph_path = os.path.join(out_dirs["graphs"], f"{video_id}_metrics_v5_4_2_lab.png")
    plot_metrics_graph(df_metrics, metrics_graph_path)

    angles_graph_path = os.path.join(out_dirs["graphs"], f"{video_id}_angles_v5_4_2_lab.png")
    ...
    bold_font_path = font_path  # Meiryo は太字も同じファイルで対応
    ...
    # タイトル
    pdf.set_font("JPB", "", 14)
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.4.2_lab）", ln=1)
    ...
    out_pdf = os.path.join(out_dirs["pdf"], f"{athlete}_form_report_v5_4_2_lab.pdf")
    pdf.output(out_pdf)
...
def main():
    parser = argparse.ArgumentParser(description="KJAC フォーム分析レポート v5.4.2_lab（高機能・実験用）")
    ...
    log(f"=== KJAC v5.4.2_lab START athlete={args.athlete}, video_id={video_id} ===")
    log(f"入力: video={args.video}, csv={args.csv}")
    log(f"ログファイル: {log_path}")

    try:
        build_pdf(args.csv, args.video, args.athlete, out_dirs)
        log("=== KJAC v5.4.2_lab END (success) ===")
    except Exception as e:
        log(f"⚠ 予期せぬエラー発生: {e}")
        log("=== KJAC v5.4.2_lab END (failed) ===")
        raise
