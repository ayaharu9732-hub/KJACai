# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_7.py
# ✅ 日本語文字完全描画＋左寄せ整列版
# ==========================================================
import os
import json
...
def create_graphs(df, athlete):
    ...
    for ax in axs:
        ax.set_xlabel("時間 [s]")
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_7.png"
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(0, 10, f"{athlete} フォーム分析レポート（v5.1.7）", new_y="NEXT", align="L")
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_7.pdf")
    pdf.output(out_path)
