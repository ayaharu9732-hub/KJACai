...
# OpenAI ライブラリ（なければ None にしてフォールバック）
try:
    import openai
except ImportError:
    openai = None
...
def extract_pose_images_from_overlay(overlay_path: str, out_dir: str) -> Dict[str, str]:
    ...
    for key, idx in indices.items():
        ...
        try:
            ...
            log(f"⚠ 画像保存に失敗: {final_path} (tmp={temp_path}) エラー: {e}")
...
# ============================================
#  テキスト整形 & 安全 multi_cell
# ============================================

def jp_wrap(text: str, max_chars: int = 38) -> str:
    """
    日本語の長文をFPDFのmulti_cellで扱いやすくするため、
    おおよその文字数で改行を入れる。
    """
    if not text:
        return ""
...
def safe_multi_cell(pdf: FPDF, text: str, h: float = 5) -> None:
    """
    FPDF の "Not enough horizontal space to render a single character"
    を絶対に出さないための安全版 multi_cell。
    1行ごとに、文字幅を見ながら自前で折り返してから multi_cell に投げる。
    """
    if not text:
        return
...
def generate_training_menu_rule(metrics: Dict[str, float],
    ...
    """
    自宅・学校でできる軽めのトレーニングを5つ返す（ルールベース）
    """
    pitch = metrics.get("pitch", float("nan"))
    stab = metrics.get("stability", float("nan"))
    com_range = metrics.get("com_y_range", float("nan"))
...
def call_openai_chat(model: str, system_prompt: str, user_prompt: str,
                     temperature: float = 0.6, max_tokens: int = 900) -> Optional[str]:
    """
    OpenAI ChatCompletion を叩くラッパー。
    openai が無い・キーが無い・エラー時は None を返す。
    """
    if openai is None:
        log("⚠ openai ライブラリがインポートできませんでした。GPT コメントはスキップします。")
        return None

    api_key = get_openai_api_key()
    if not api_key:
        log("⚠ OPENAI_API_KEY が設定されていません。GPT コメントはスキップします。")
        return None
...
def build_metrics_summary_for_prompt(metrics: Dict[str, float],
                                     angles: Dict[str, float],
                                     athlete: str) -> str:
    ...
    lines.append("=== メトリクス ===")
    lines.append(f"ピッチ(歩/秒): {fmt(metrics.get('pitch', float('nan')))}")
    lines.append(f"ストライド(m): {fmt(metrics.get('stride', float('nan')))}")
    lines.append(f"安定性スコア(0-10): {fmt(metrics.get('stability', float('nan')))}")
    lines.append(f"平均速度(m/s): {fmt(metrics.get('speed_mean', float('nan')))}")
    lines.append(f"最大速度(m/s): {fmt(metrics.get('speed_max', float('nan')))}")
    lines.append(f"重心上下動レンジ(m): {fmt(metrics.get('com_y_range', float('nan')), 3)}")
    lines.append("")
    lines.append("=== 関節角度・体幹 ===")
    lines.append(f"膝角度平均(度): {fmt(angles.get('knee_flex_mean', float('nan')))}")
    lines.append(f"膝角度レンジ(度): {fmt(angles.get('knee_flex_range', float('nan')))}")
    lines.append(f"股関節角度平均(度): {fmt(angles.get('hip_flex_mean', float('nan')))}")
    lines.append(f"股関節角度レンジ(度): {fmt(angles.get('hip_flex_range', float('nan')))}")
    lines.append(f"体幹前傾平均(度): {fmt(angles.get('trunk_tilt_mean', float('nan')))}")
    lines.append(f"体幹前傾レンジ(度): {fmt(angles.get('trunk_tilt_range', float('nan')))}")
    return "\n".join(lines)


def generate_ai_comments_pro_gpt(metrics: Dict[str, float],
                                 angles: Dict[str, float],
                                 athlete: str) -> Optional[str]:
    """
    プロ・コーチ向けコメント（gpt-4.1）
    """
    summary = build_metrics_summary_for_prompt(metrics, angles, athlete)

    system_prompt = (
        "あなたは陸上短距離の専門コーチです。"
        "入力されるデータ（ピッチ、ストライド、速度、安定性スコア、関節角度など）をもとに、"
        "フォームの技術的な特徴と改善ポイントを、専門家向けに日本語で解説してください。"
        "・200〜400文字程度\n"
        "・具体的な動き（接地、遊脚、骨盤、体幹の使い方など）に言及する\n"
        "・最後に『今後の重点ポイント』を2〜3個、箇条書きにする\n"
        "・文体は「〜です」「〜します」で統一\n"
    )

    user_prompt = (
        "以下が選手の計測データです。この情報をもとに、専門コーチ向けのフォーム分析コメントを作成してください。\n\n"
        f"{summary}"
    )
...
def generate_ai_comments_easy_gpt(metrics: Dict[str, float],
                                  ...
                                  athlete: str) -> Optional[str]:
    """
    中学生・保護者向けコメント（gpt-4.1-mini）
    """
    summary = build_metrics_summary_for_prompt(metrics, angles, athlete)

    system_prompt = (
        "あなたは中学生の陸上部を指導するコーチです。"
        "入力されるデータ（ピッチ、ストライド、速度、安定性スコア、関節角度など）をもとに、"
        "走りの良い点と「これから伸ばしたい点」を、中学生と保護者にもわかりやすい言葉で日本語説明してください。"
        "・200〜350文字程度\n"
        "・専門用語はできるだけかみ砕いて説明\n"
        "・最後に『今日から意識してほしいこと』を2つ、箇条書きにする\n"
        "・文体は「〜です」「〜ましょう」で統一\n"
    )

    user_prompt = (
        f"選手名: {athlete}\n"
        "以下が計測データです。これをもとに、中学生・保護者向けのフォーム解説コメントを作ってください。\n\n"
        f"{summary}"
    )
...
def parse_numbered_blocks(text: str, expected: int = 5) -> List[str]:
    """
    GPTが返したテキストを「①〜」「②〜」単位で分割。
    うまく分割できなければ全体を1ブロックとして返す。
    """
    if not text:
        return []
...
def generate_training_menu_gpt(metrics: Dict[str, float],
                               angles: Dict[str, float]) -> Optional[List[str]]:
    """
    トレーニングメニュー（gpt-4.1-mini）
    """
    summary = build_metrics_summary_for_prompt(metrics, angles, athlete="(匿名)")

    system_prompt = (
        "あなたは陸上短距離のコーチです。"
        "入力されるデータから、選手の特徴と弱点を踏まえて、"
        "自宅や学校でできる簡単なトレーニングメニューを5つ提案してください。"
        "対象は中学生で、器具はほぼ使えない想定です。\n"
        "・それぞれのメニューは、タイトル＋1〜3文の説明\n"
        "・走りのフォームにつながる内容にする\n"
        "・きつすぎない内容にする\n"
        "・日本語で書く\n"
        "・①〜⑤の番号を先頭につける\n"
        "・出力は説明テキストだけ（余計なコメントは書かない）\n"
    )

    user_prompt = (
        "以下の選手データをもとに、トレーニングメニューを5つ提案してください。\n\n"
        f"{summary}"
    )
    ...
    blocks = parse_numbered_blocks(text, expected=5)
...
def build_pdf(csv_path: str,
    ...
    # torso tilt を metrics 側に反映（グラフ表示用）
    if df_angles is not None and "torso_tilt_deg" not in df_metrics.columns:
        trunk_cols = [c for c in df_angles.columns if any(k in c.lower() for k in ["trunk", "torso_tilt", "body_tilt"])]
        if trunk_cols:
            try:
                if "frame" in df_metrics.columns and "frame" in df_angles.columns:
                    merged = pd.merge(
                        df_metrics,
                        df_angles[["frame"] + trunk_cols],
                        on="frame",
                        how="left",
                        suffixes=("", "_ang"),
                    )
                    merged["torso_tilt_deg"] = merged[trunk_cols].mean(axis=1, skipna=True)
                    df_metrics = merged
                else:
                    # 行数がほぼ同じ前提で、そのまま平均を入れる
                    trunk_series = df_angles[trunk_cols].mean(axis=1, skipna=True)
                    trunk_series = trunk_series.reindex(df_metrics.index, method="nearest")
                    df_metrics["torso_tilt_deg"] = trunk_series.values
                log("metrics に torso_tilt_deg を追加（angles から推定）")
            except Exception as e:
    ...
    # 専門向けコメント（GPT or ルールベース）
    pdf.set_font("JPB", "", 11)
    pdf.cell(0, 8, "AIフォーム解析コメント（専門向け）", ln=1)
    pdf.set_font("JP", "", 9)
    safe_multi_cell(pdf, pro_comment, h=5)
    pdf.ln(2)
    ...
    pdf.set_font("JP", "", 9)
    safe_multi_cell(pdf, easy_comment, h=5)
    pdf.ln(2)
...
def main():
    parser = argparse.ArgumentParser(description=f"KJAC フォーム分析レポート {VERSION_STR}（GPT対応版）")
