...
VERSION_STR = "v5.5.1_gpt_fix"
LOG_FILE: Optional[str] = None
client = OpenAI()  # OPENAI_API_KEY は環境変数から読む
...
def get_root_output_dirs(athlete: str, video_id: str) -> Dict[str, str]:
    ...
    for d in [base, pdf_dir, graphs_dir, pose_dir, angles_dir, overlay_dir, logs_dir]:
        ensure_dir(d)
...
def plot_metrics_graph(df_metrics: pd.DataFrame, out_path: str) -> None:
    video_id = os.path.splitext(os.path.basename(out_path))[0]
...
def extract_pose_images_from_overlay(overlay_path: str, out_dir: str) -> Dict[str, str]:
    ...
    for key, idx in indices.items():
        ...
        temp_path = os.path.join(temp_dir, f"pose_{key}_{uuid4().hex}.png")
...
def dump_overlay_frames(overlay_path: str, out_dir: str, step_sec: float = 0.1) -> int:
    """
    オーバーレイ動画を 0.1秒ごとに連続PNG保存。
    ファイル名: frame_0000.png, frame_0001.png, ...
    """
    if not os.path.exists(overlay_path):
        log(f"⚠ overlay連続画像: 動画が見つかりません: {overlay_path}")
        return 0
    ...
    while True:
        ...
        if frame_idx % step_frames == 0:
            temp_path = os.path.join(temp_dir, f"overlay_{uuid4().hex}.png")
            ok = cv2.imwrite(temp_path, frame)
            if not ok:
                log(f"⚠ overlay連続画像: 一時保存失敗 frame_idx={frame_idx}")
            else:
                final_name = f"frame_{save_idx:04d}.png"
    ...
    cap.release()
    log(f"overlay連続フレーム出力: {save_idx}枚 -> {out_dir}")
    return save_idx


# ============================================
#  テキスト整形 & safe_multi_cell
# ============================================

def jp_wrap(text: str, max_chars: int = 38) -> str:
    """
    日本語の長文を、ある程度の文字数で手動改行。
    GPTから来た長い一行でも、安全に multi_cell に流せるようにする。
    """
    if not text:
        return ""
...
def safe_multi_cell(pdf: FPDF, text: str, h: float = 5.0) -> None:
    """
    FPDF の "Not enough horizontal space to render a single character"
    を避けるためのラッパ。
    - jp_wrap である程度改行を入れてから multi_cell
    - さらに、1行ごとに文字幅を見て分割する保険付き
    """
    if not text:
        return

    text = text.replace("\r", "")
    text = jp_wrap(text, max_chars=38)
    ...
    for para in text.split("\n"):
        ...
        for ch in line:
            ...
            if w <= max_width:
                ...
                if buf == "":
                    # 1文字も入らない → 仕方ないのでそのまま出す
                    pdf.multi_cell(0, h, ch)
                    buf = ""
                else:
...
def generate_ai_comments_easy_rule(metrics: Dict[str, float],
    ...
    return jp_wrap(" ".join(lines), max_chars=34)
...
def build_gpt_inputs(metrics: Dict[str, float],
    ...
    speed_mean = metrics.get("speed_mean")
    speed_max = metrics.get("speed_max")
    com_range = metrics.get("com_y_range")

    knee_mean = angles.get("knee_flex_mean")
    hip_mean = angles.get("hip_flex_mean")
    trunk_mean = angles.get("trunk_tilt_mean")
    ...
    lines = [
        ...
        f"ストライド: {stride:.3f} m",
        f"安定性スコア: {stab:.2f} / 10",
        f"平均速度: {speed_mean:.3f} m/s",
        f"最大速度: {speed_max:.3f} m/s",
        f"重心上下動レンジ: {com_range:.4f} m",
        f"膝関節平均角度: {knee_mean:.3f} 度",
        f"股関節平均角度: {hip_mean:.3f} 度",
        f"体幹前傾平均: {trunk_mean:.3f} 度",
    ]
    return "\n".join(lines)
...
def generate_ai_comments_pro_gpt(metrics: Dict[str, float],
                                 ...
                                 athlete: str) -> str:
    system_prompt = (
        "あなたは陸上競技（短距離・中長距離）の専門コーチです。"
        "数値データに基づいて、フォームの良い点と改善点を、"
        "日本語で専門家向けに解説してください。"
        "ただし1ページのPDFに収まるように、"
        "全体で800〜900文字程度にまとめてください。"
    )
    user_prompt = (
        "以下の疾走フォームデータをもとに、技術分析中心のコメントを書いてください。\n"
        "・良い点\n"
        "・改善が必要な点\n"
        "・特に意識してほしいキーポイント（3つ程度）\n"
        "を含めてください。\n\n"
        "【データ】\n" + build_gpt_inputs(metrics, angles, athlete)
    )

    text = call_openai_chat("gpt-4.1", system_prompt, user_prompt, max_tokens=900)
    if not text:
...
def generate_ai_comments_easy_gpt(metrics: Dict[str, float],
                                  ...
                                  athlete: str) -> str:
    system_prompt = (
        "あなたは中学生の陸上部を指導する優しいコーチです。"
        "フォームのデータをもとに、中学生と保護者にもわかる言葉で、"
        "走りの良いところと、今後の課題を説明してください。"
        "口調は『〜です』『〜しましょう』など柔らかく、"
        "全体で600〜800文字程度にしてください。"
    )
    user_prompt = (
        "以下の疾走フォームデータをもとに、"
        "中学生・保護者向けのコメントを書いてください。\n"
        "・今の走りの良いところ\n"
        "・これから練習で意識するとよいポイント\n"
        "・前向きになれる一言メッセージ\n"
        "を含めてください。\n\n"
    ...
    text = call_openai_chat("gpt-4.1-mini", system_prompt, user_prompt, max_tokens=700)
    if not text:
...
def generate_training_menu_gpt(metrics: Dict[str, float],
    ...
    user_prompt = (
        "以下の疾走フォームデータをもとに、"
        "フォーム改善に役立つトレーニングメニューを5個考えてください。\n"
        "・各メニューは、タイトル＋やり方の説明（2〜3行）\n"
        "・難易度は中学生が無理なくできるレベル\n"
        "・走りのどのポイントに効くかも一言添えてください。\n\n"
    ...
    # GPT から来たテキストを「5個のドリル」にざっくり分割
    # 1行目の「①」「1.」などで分かれている想定だが、
    # シンプルに空行で区切る。
...
def build_pdf(csv_path: str,
    ...
    # PDF 用 3枚 & overlay 連続フレーム
    pose_images = extract_pose_images_from_overlay(overlay_path, out_dirs["pose_images"])
    dump_overlay_frames(overlay_path, out_dirs["overlay"], step_sec=0.1)
    ...
    if pose_images:
        ...
        for i, key in enumerate(order):
            ...
            try:
                ...
                log(f"⚠ PDF への骨格画像貼り付けに失敗: {pose_images[key]} エラー: {e}")
    ...
    if os.path.exists(metrics_graph_path):
        try:
            ...
            log(f"⚠ メトリクスグラフの貼り付けに失敗: {metrics_graph_path} エラー: {e}")
    ...
    if angles_graph_ok and os.path.exists(angles_graph_path):
        try:
            ...
            log(f"⚠ 角度グラフの貼り付けに失敗: {angles_graph_path} エラー: {e}")
