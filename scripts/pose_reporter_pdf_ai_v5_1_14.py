# ==========================================================
# 🧩 pose_reporter_pdf_ai_v5_1_14.py
# ✅ グラフ日本語フォント完全対応 (IPAexGothic)
# ✅ タイトル白豆腐対策 + 軸文字/ラベルも黒
# ==========================================================
import os
import json
...
def create_graphs(df, athlete):
    ...
    for ax, col, title, color in zip(axs, cols, titles, colors):
        ...
        # ⬇ タイトル・軸ラベル全てに fontproperties を明示適用
        ax.set_title(title, fontsize=12, fontproperties=prop, color="black", pad=10)
        ax.set_xlabel("時間 [s]", fontsize=10, fontproperties=prop, color="black")
        ax.set_ylabel("", fontsize=10, fontproperties=prop, color="black")
    ...
    plt.tight_layout()
    path = f"outputs/{athlete}_graphs_v5_1_14.png"
...
def build_pdf(csv_path, video_path, athlete):
    ...
    pdf.cell(page_w, 12, f"{athlete} フォーム分析レポート（v5.1.14）",
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{athlete}_form_report_v5_1_14.pdf")
    pdf.output(out_path)
    print(f"✅ PDF出力完了: {out_path}")
    print("🖤 グラフ日本語タイトル・軸ラベル完全黒文字で描画済み")
