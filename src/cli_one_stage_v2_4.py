# -*- coding: utf-8 -*-
"""
cli_one_stage_v2_4.py
Start(0–20m)安定完全版（フレーム画像確実埋め込み）
"""
...
def _read_cfg(p): import yaml; return yaml.safe_load(open(p, encoding="utf-8"))
...
def _iter_times(start,total,step):
    t=max(0.0,start)
    while t<=total+1e-9: yield round(t,3); t+=step

def _kalman_like(v):
    a=0.3; y=[]
    for i,x in enumerate(v): y.append(x if i==0 else a*x+(1-a)*y[-1])
    return np.array(y)

def _parse_filename(path):
    base=os.path.splitext(os.path.basename(path))[0].replace(" ","").replace("_","")
    m=re.match(r"^([^\d]+)([\d.]+)$",base)
    if m: return m.group(1),m.group(2).rstrip(".")
    return base,""
...
def make_pdf_one_stage(pdf_path,pages,times,speeds_ms,cfg,label,ai_summary_text):
    ...
    try: pdfmetrics.registerFont(TTFont("IPAexGothic",font_path)); font_jp="IPAexGothic"
    except: font_jp="Helvetica"

    c=canvas.Canvas(pdf_path,pagesize=landscape(A4))
    W,H=landscape(A4)

    def _draw_image_safe(img_path,x,y,w=360,h=110):
        try: c.drawImage(img_path,x,y,width=w,height=h,preserveAspectRatio=True)
        except Exception as e: print("[WARN] 画像描画失敗:",e)

    def draw_speed_plot():
        import matplotlib.pyplot as plt
        if len(times)==0 or len(speeds_ms)==0: return
        fig=plt.figure(figsize=(8,3),dpi=150)
        plt.plot(times,speeds_ms)
        plt.xlabel("Time (s)"); plt.ylabel("Speed (m/s)")
        plt.title("速度プロファイル"); plt.tight_layout()
    ...
    def draw_paragraph(x,y,text,width=760,size=12):
        styles=getSampleStyleSheet(); st=styles["Normal"]
        st.fontName=font_jp; st.fontSize=size; st.leading=18
        p=Paragraph(str(text).replace("\n","<br/>"),st)
        w,h=p.wrap(width,9999); p.drawOn(c,x,y-h); return h

    c.setFont(font_jp,20); c.drawString(40,H-60,f"動画レポート：{label}")
    y=H-90
    for (t,phase,img,v_ms,v_kmh) in pages:
        ...
        y-=140
        if y<160: c.showPage(); c.setFont(font_jp,20); c.drawString(40,H-60,f"動画レポート：{label}"); y=H-90
    c.showPage()
    draw_speed_plot()
    c.setFont(font_jp,18); c.drawString(40,H-60,"AI分析レポート（サマリー）")
...
def main():
    ...
    ap.add_argument("--video"); ap.add_argument("--pick-dialog",action="store_true")
    ...
    if args.pick_dialog or not video:
        ...
        r=tk.Tk(); r.withdraw()
        video=filedialog.askopenfilename(initialdir=cfg.get("pick_default_dir","videos"),filetypes=[("Video","*.mp4 *.mov *.avi")])
        r.destroy()
    if not video: raise FileNotFoundError("動画が選択されていません")
    ...
    if not cap.isOpened(): raise RuntimeError("動画を開けません")
    fps=cap.get(cv2.CAP_PROP_FPS) or 30.0
    total=cap.get(cv2.CAP_PROP_FRAME_COUNT)/fps
    start=cfg.get("start_sec",0.0); step=cfg.get("step_sec",0.1)
    ok0,f0=_frame_at_time(cap,start,fps)
    prev=cv2.cvtColor(f0,cv2.COLOR_BGR2GRAY); prev_pts=None

    times=[]; speeds=[]; pages=[]; vpeak=0
    for t in _iter_times(start,total,step):
        ...
        prev=gray
        v_mps=float(v_px)*float(mpp); v_kmh=v_mps*3.6
        vpeak=max(vpeak,v_px)
        phase=classify_phase(v_px,vpeak,None,None)
        draw_guides(fr); put_label(fr,f"{args.label} | {phase} | t={t:.1f}s")
        img=os.path.join(frames_dir,f"t_{t:.1f}.png".replace(".","_"))
        Image.fromarray(cv2.cvtColor(fr,cv2.COLOR_BGR2RGB)).save(img)
        pages.append((t,phase,img,v_mps,v_kmh))
        times.append(t); speeds.append(v_mps)
    cap.release()

    speeds_s=_kalman_like(speeds)
    ai_summary=generate_ai_summary(times=list(times),speeds_ms=list(speeds_s),
