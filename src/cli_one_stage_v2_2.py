# -*- coding: utf-8 -*-
"""
cli_one_stage_v2_2.py
Start(0–20m)専用・安定完全版
・距離入力あり
・画像確実埋め込み（OpenCV→PIL→PNG変換）
...
def make_pdf_one_stage(pdf_path, pages, times, speeds_ms, cfg, label, ai_summary_text):
    ...
    def _draw_image_safe(c, img_path, x, y, width=360, height=110):
        """OpenCV JPEGを安全にPDFへ埋め込む（PNG変換対応）"""
        import tempfile
        try:
            ...
            with Image.open(img_path) as pil_img:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                    ...
                    tmp_path = tmp.name
            c.drawImage(tmp_path, x, y, width=width, height=height, preserveAspectRatio=True)
            os.remove(tmp_path)
